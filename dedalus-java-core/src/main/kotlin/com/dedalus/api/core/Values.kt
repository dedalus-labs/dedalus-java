...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { DedalusInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw DedalusInvalidDataException("`$name` is not set")
            is JsonNull -> throw DedalusInvalidDataException("`$name` is null")
            else -> throw DedalusInvalidDataException("`$name` is invalid, received $this")
        }
